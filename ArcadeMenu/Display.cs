 ...
 * Date: 3/20/2020
 * Time: 6:45 PM
 * Class created by Yosh:  https://stackoverflow.com/users/6764710/yosh - 9/2/2016
 * Added GetCurrentRotate function nin2k - 3/20/2020
 * Added MointorPixelStart function nin2k - 3/20/2020 
 *
 */
using System;
...
namespace ArcadeMenu
{
    public class Display
    {
        ...
        public static int GetCurrentRotate(uint DisplayNumber)
        {
            if (DisplayNumber == 0)
                throw new ArgumentOutOfRangeException("DisplayNumber", DisplayNumber, "First display is 1.");
            ...
            if (0 != NativeMethods.EnumDisplaySettings(
                d.DeviceName, NativeMethods.ENUM_CURRENT_SETTINGS, ref dm))
            {
                result = dm.dmDisplayOrientation;
            }

            return result;
        }

        public static int MointorPixelStart(uint DisplayNumber)
        {
            int pixelx = 0;
            foreach (var screen in Screen.AllScreens)
            {
                int GetScreen = 0;
                bool dispno = int.TryParse(screen.DeviceName.Substring(screen.DeviceName.Length - 1), out GetScreen);
                GetScreen = GetScreen - 1;
                if (dispno == true && GetScreen == DisplayNumber)
                {
                    pixelx = screen.Bounds.X + 1;
                }
            }
            if (pixelx != 0)
            {
